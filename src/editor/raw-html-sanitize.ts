...
const ATTR_RULES: Record<string, Record<string, (value: string) => string | null>> = {
  img: {
    src: sanitizeUrl,
    alt: (v) => v,
    title: (v) => v,
...
export function renderWhitelistedHtml(raw: string): string | null {
  ...
  const firstElement = Array.from(body.childNodes).find((n) => n.nodeType === Node.ELEMENT_NODE) as
    | Element
    | undefined;
  if (!firstElement || !WHITELISTED_TAGS.has(firstElement.tagName.toLowerCase())) return null;
