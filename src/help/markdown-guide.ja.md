...
行頭に `#` とスペースを入力するとレベル1、`##` とスペースならレベル2の見出しになります。上の見出しが実際の表示例です。
...
**設定 → 編集と外観 → 画像**では、今後貼り付ける画像を自分の画像ホストへ送信するように設定できます。エンドポイントは `multipart/form-data` の `file` フィールドで画像を受信し、公開 URL をプレーンテキストまたは設定可能な JSON フィールドで返します。ファイル名は自動変更、元の名前の保持、アップロード前の毎回確認から選べます。保存先を切り替えても既存の画像参照は変更されません。
