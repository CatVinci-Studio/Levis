...
<p align="center">
  <strong>一款 Markdown 编辑器 —— AI Agent 会悄悄参与其中。</strong><br>
  在干净、无干扰的画布上写作；Agent 只在该出现的时候才出现，不会碍事。
</p>
...
Levis 首先是一个 Markdown 编辑器。标题、粗体/斜体、公式等语法会直接渲染，而不是显示原始符号 —— 只有点进去编辑时，Typora 风格的原始标记才会重新出现。
