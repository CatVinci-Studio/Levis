...
<p align="center">
  <strong>Typora 风格的所见即所得 Markdown 编辑器，内置 AI 写作助手。</strong><br>
  在干净、无干扰的画布上写作 —— 原始语法只有在光标停留时才会显示。
</p>
...
一款跨平台桌面 Markdown 编辑器，基于 [Milkdown](https://milkdown.dev/)（基于 ProseMirror 的编辑器框架）、React 和 [Tauri 2](https://tauri.app/) 构建。标题、粗体/斜体、公式等语法会直接渲染，而不是显示原始符号 —— 只有点进去编辑时，Typora 风格的原始标记才会重新出现。内置的 AI 助手可以在你打字时续写、标记语法问题，并回答关于当前文档内容的问题。

## 为什么做这个

- **默认无干扰。** 没有 `**`/`#`/`$...$` 这些原始符号的干扰 —— 所见即所得，就地编辑。
- **自带模型任你选。** 使用 ChatGPT (Codex) 或 Claude 登录，粘贴普通的 OpenAI API Key，或者指向任意兼容 OpenAI 接口的服务（包括本地模型）—— 按提供方选择模型、测试连接，就这么简单。
- **能读懂文档的 Agent。** 就正在写的内容提问；在 Codex 模式下，它可以通过真正的工具调用去搜索文档，而不是瞎猜。
