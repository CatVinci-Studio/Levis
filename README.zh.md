...
> [!IMPORTANT]
> **macOS 提示「Levis 已损坏，无法打开」** —— 应用暂未签名（目前还没预算购买 Apple 开发者账号），Gatekeeper 会拦截下载的安装包。应用本身是安全的，执行一次以下命令清除隔离标记即可正常打开：
>
> ```sh
...
现代的所见即所得 Markdown 编辑器往往只专注于内容本身，缺少更现代化的写作辅助工具。目前主要有以下几个问题：

1. 像 Typora 这类编辑器，内存占用较大、速度较慢。
2. 直接用 VS Code 或 Neovim 写 Markdown，往往需要分屏才能预览，无法真正做到所见即所得，这会带来额外的写作负担，让人无法完全专注。
3. 有些 AI Agent 在编辑时存在感太强，会干扰人对写作本身的判断。
